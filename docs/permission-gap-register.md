...
| GAP-004 | `/api/cases/[id]/finalization/*` (except `red-number` and `finalize`) | Authenticated users | Strict mutation boundaries for finalization steps | Not comprehensively protected by `requireApiPermission` across all related mutations | C (Medium) | Explicitly apply `requireApiPermission` or `hasPermission` to all finalization APIs | No (Deferred per prompt) |
